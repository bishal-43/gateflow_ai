/**
 * EntryScan.jsx — Guard entry scanning page (Redesigned for Phase 2).
 *
 * API: POST /entry/scan → { qr_token, gate_id }
 * Response: EntryScanResponse { status, visitor_name, session_id, ... }
 *
 * Roles: GUARD | ADMIN
 */
...
import { useEntryScan } from '@/hooks/useEntry'
import { friendlyScanError } from '@/lib/scanMessages'
import { cn, formatDateTime } from '@/lib/utils'
import {
  CheckCircle, XCircle, Camera, CameraOff, DoorOpen, RefreshCw, ArrowLeft
} from 'lucide-react'

export default function EntryScan() {
  const entryScan = useEntryScan()
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      html5QrRef.current = new Html5Qrcode('qr-reader-entry')
  ...
  return (
    <DashboardLayout title="Entry scan" subtitle="Scan the guest QR — records entry at the gate you pick">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {scanResult && (
          ...
          >
            ...
            <h2 className={cn(
              ...
              {scanResult.type === 'success' ? '✓ ENTRY ALLOWED' : '✗ ENTRY BLOCKED'}
            </h2>
            ...
            {scanResult.type === 'success' && scanResult.data && (
              <div className="mt-6 space-y-3">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-300">
                  📍 Gate: {scanResult.data.gate_id ?? selectedGate}
                </p>
                {scanResult.data.allowed_until && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Valid until: {formatDateTime(scanResult.data.allowed_until)}
                  </p>
                )}
              </div>
            )}
        ...
        <div className="mx-auto max-w-xl rounded-2xl border-2 border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <label htmlFor="gate-select" className="mb-2 block text-base font-semibold text-gray-900 dark:text-gray-100">
          ...
          <select
            ...
            className="w-full min-h-[52px] rounded-xl border-2 border-gray-300 bg-white px-4 text-lg font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
          >
            {['Gate 1', 'Gate 2', 'Gate 3', 'Main Gate', 'Back Gate'].map((g) => (
        ...
        {!scanResult ? (
          <div className="space-y-4">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gray-900 aspect-square max-h-96 flex items-center justify-center shadow-lg">
              {scanning ? (
                <>
                  <div id="qr-reader-entry" className="w-full h-full" />
                  {/* Scanning overlay */}
                  <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-56 w-56 border-4 border-green-400 rounded-2xl">
                      {['tl', 'tr', 'bl', 'br'].map((corner) => (
                        <div key={corner} className={cn(
                          'absolute h-8 w-8 border-4 border-green-400',
                ...
                <div className="flex flex-col items-center gap-4 text-gray-300">
                  <DoorOpen className="h-24 w-24" aria-hidden />
                  <p className="text-xl font-bold">Ready — tap green button</p>
                </div>
              )}
            </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <span>Recent Entries ({scanHistory.length})</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
